...
void rosync_task(void)
{
    ...
    switch (rosync_statemachine)
    {
        ...
        case ROSYNC_SM_DOWNLOADDESC:
            if (radio.textAvail())
            {
                rosync_downloadDescChunk(radio.textReadPtr(true));
            }
            if ((millis() - rosync_lastRxTime) >= 1000)
            {
                ...
                state = ROSYNC_SM_NODESC_ERR;
            }
            break;
        case ROSYNC_SM_DOWNLOADNVM:
            if (radio.textAvail())
            {
                rosync_downloadNvmChunk(radio.textReadPtr(true));
            }
            if ((millis() - rosync_lastRxTime) >= 1000)
            {
                state = ROSYNC_SM_NOSYNC_ERR;
            }
            break;
...
void rosync_downloadDescChunk(char* str)
{
    if (str[0] = 'D' && str[1] = 'D' )
    {
...
void rosync_downloadNvmChunk(char* str)
{
    if (rosync_nvm == NULL)
    {
        return;
...
bool rosync_loadDescFile(uint32_t id)
{
    ...
    rosync_menu = new RoachMenuCfgLister(MENUID_CONFIG_ROBOT, "ROBOT CFG", "robot", (void*)rosync_nvm, roach_nvm_gui_desc_t* rosync_desc_tbl);
    if (rosync_menu == NULL) {
